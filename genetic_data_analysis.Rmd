...
#The color grid is defined manually, to show each EC class in the same color. 
...
highlight.sector(c(paste0("R", 1:6)), track.index = 2, col = "#A8B6CC", 
    text = "Reactome", cex = 0.6, text.col = "white", niceFacing = TRUE)
highlight.sector(c(paste0("W", 1:6)), track.index = 2, col = "#1F449c", 
    text = "WikiPathways", cex = 0.6, text.col = "white", niceFacing = TRUE)
highlight.sector(c(paste0("G", 1:6)), track.index = 2, col = "#F05039", 
    text = "Gene Ontology", cex = 0.6, text.col = "white", niceFacing = TRUE)

#Add legend with EC code classification
tab <- as.data.frame(
  c(
...
## Add legend for EC-code classification.
...
# EC 5: Isomerases
# EC 6: Ligases
# EC 7: Translocases --> Not found?

```

```{r}
##Save the figure in PNG file suited for paper:
png(filename="overlap_Genes_chordDiagram.png", width = 350, height = 350, units = "px") 
...
##Report unclassified, report missing translocase (nr 7).
