...
speciesPaper <- c("hsapiens_gene_ensembl", "celegans_gene_ensembl", "dmelanogaster_gene_ensembl", "mmusculus_gene_ensembl")
...
colnames(lipidGenes) <- c("Gene.name", "mouse.occurrence", "celegans.occurrence", "dmelanogaster.occurrence")
...
#rm(list=setdiff(ls(), "lipidGenes"))
```

Visualize the homology mapped data
```{r}
##Transpose data for visualization:
##Count nr of genes with NA(=0), 1, 2, 3... mappings; color is species  
highestOccurence.mouse <- max(lipidGenes$mouse.occurence, na.rm = TRUE)
missingOccurence.mouse <- sum(is.na(lipidGenes$mouse.occurence))
highestOccurence.celegans <- max(lipidGenes$celegans.occurence, na.rm = TRUE)
missingOccurence.celegans <- sum(is.na(lipidGenes$celegans.occurence))
highestOccurence.dmelanogaster <- max(lipidGenes$dmelanogaster.occurence, na.rm = TRUE)
missingOccurence.dmelanogaster <- sum(is.na(lipidGenes$dmelanogaster.occurence))

##Create summary of occurence for whole gene list, and add column with species name
...
#Merge the data in one dataframe:
Data_overview <- bind_rows(occurrence.Mouse.Summary, occurrence.celegans.Summary, occurrence.dmelanogaster.Summary) %>% arrange(Var1)
colnames(Data_overview) <- c("Mappings", "Occurrence", "species")

library(ggplot2)

# A basic scatterplot with color depending on Species
ggplot(Data_overview, aes(x = Mappings, y = Occurrence, color = species)) + 
    geom_point(size=6) #+
    #theme_ipsum()
