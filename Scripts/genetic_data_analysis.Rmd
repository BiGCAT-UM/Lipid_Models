...
mouse.occurence <- human2mouseLipids.df %>% count(Gene.name) 
celegans.occurence <- human2celegansLipids.df %>% count(Gene.name) 
dmelanogaster.occurence <- human2dmelanogasterLipids.df %>% count(Gene.name) 

#Merge the data in one dataframe:
lipidGenes <- left_join(lipidGenes, mouse.occurence, by = "Gene.name") %>%
                left_join(., celegans.occurence, by='Gene.name') %>%
                left_join(., dmelanogaster.occurence, by='Gene.name')
colnames(lipidGenes) <- c("Gene.name", "mouse.occurence", "celegans.occurence", "dmelanogaster.occurence")
